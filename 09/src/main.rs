...
#[inline(always)]
fn for_each_parent_pair<F>(child: usize, len: usize, mut f: F)
where
    F: FnMut(usize, usize),
{
    for p1 in 0..len {
...
fn part1(input: &str) -> UInt {
    ...
    for (c, (_, child_dna)) in sequences.iter().enumerate() {
        let mut found_similarity = None;
        for_each_parent_pair(c, sequences.len(), |p1, p2| {
            if found_similarity.is_some() {
                return;
            }
            ...
            let s = similarity(parent1_dna, parent2_dna, child_dna);
            if s > 0 {
                found_similarity = Some(s);
            }
        });

        if let Some(s) = found_similarity {
            return s;
        }
    }
    0
...
fn part3(input: &str) -> usize {
    ...
    for c in 0..n {
        let (child_id, child_dna) = &sequences[c];
        let mut found_parents = false;

        for_each_parent_pair(c, n, |p1, p2| {
            if found_parents {
                return;
            }
